...
void ComputeCore()
{
    ...
    for(int j=0; j<n; ++j)
    {
        for(int i=0; i<n; ++i)
        {
            ...
            dist[j][i] = 16.0 + Dist(com[i], com[j]);
        }
    }
...
void Compute() // get input and then call ComputeCore(); then print output
{
    ...
    while(true){
        ...
        for(int i=0; i<n-1; i++)
        {
            ...
            cout << com[best_sol[i]].x << "," << com[best_sol[i]].y << ") to (";
            cout << com[best_sol[i+1]].x << "," << com[best_sol[i+1]].y << ") is " ;
            cout << fixed << setprecision(2) << dist[best_sol[i]][best_sol[i+1]] << " feet." << endl;
        }
...
int main()
{
	//freopen("input_00216.txt", "r", stdin);
    //freopen("output_00216.txt", "w", stdout);
